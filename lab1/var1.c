...
static int __init proc_var1_init(void)
{	
	int ret = 0;
...
static void __exit proc_var1_exit(void)
{
	proc_remove(entry);
	cdev_del(cdevp);
	unregister_chrdev_region(devno,COUNT);
...
module_init(proc_var1_init);
module_exit(proc_var1_exit);
